...
## Figuras de matplotlib como objetos

Comenzamos creando una figura en la variable `fig`, y a partir de ella creamos una instancia de ejes o `axes` usando el método `add_axes`:

```{code-cell} ipython3
fig = plt.figure() # creamos la figura y la guardamos en fig
axes = fig.add_axes([0.1, 0.1, 0.8, 0.8]) # izquierda, inferior, ancho, alto (rango 0 a 1)
x = np.linspace(-1,1,51)
y = x**2
axes.plot(x, y, 'r') # dibujamos una línea roja
axes.set_xlabel('x') # etiqueta eje x
axes.set_ylabel('y') # etiqueta eje y
axes.set_title('title'); # titulo de la figura
```

Notar que tenemos el control total de dónde se colocan los ejes de trazado. Podemos agregar fácilmente más de un eje a la figura:
...
Ahora que hemos cubierto los conceptos básicos de cómo crear una figura y agregar  ejes a la misma, veamos cómo decorar una figura con títulos, etiquetas de ejes y leyendas.
...
La mejor manera de agregar  una leyenda es haciendo uso de la palabra clave `label="label text"` cuando se agregan sub-figuras u otros objetos a los ejes, y luego usar el método `legend` sin argumentos para agregar la leyenda a la figura:
